...
Vamos estabelecer alguns fatos básicos de Teoria dos Grafos, caracterizar árvores nesse contexto e oferecer um resultado com duas condições razoavelmente fracas e suficientes para estabelecer que um grafo é uma árvore. Esse resultado tem implicações diretas em como estruturamos uma árvore de decisão, implicações estas que nos levam a estender naturalmente nossa noção de grafo árvore para uma função que relaciona um espaço de mensuração $\X$ e um de resposta $\Y$.
...
\begin{defi}
Um grafo $G$ é dito uma \textbf{árvore} se para quaisquer dois vértices de $G$ existe um caminho único os ligando. Se há um vértice $v$ de $G$ tal que existe um caminho entre todo outro vértice $u$ e $v$, então chamamos $v$ de a \textbf{raiz} da árvore. Os vértices com grau maior que $1$ que não são a raiz são chamados \textbf{nodos} e os com grau unitário são chamados de \textbf{folhas}. Um conjunto disjunto de árvores é dito uma \textbf{floresta}.
\end{defi}
...
A ausência de ciclos impõe que todo teste lógico que associaremos é \textit{completo} no sentido de que toda observação pode ser avaliada e terá alguma resposta. Alguns exemplos de testes lógicos completos são: uma pessoa tem mais que $a$ cm de altura, um avião tem mais de $b$ metros de comprimento, uma pessoa ganha mais de $c$ reais por mês, o estado de nascimento de alguém é ou não o RJ, etc. Testes completos não "misturam" regras de previsão/classificação. Assim nenhuma observação pode seguir um caminho diferente entre a raiz e sua folha. 
...
Um procedimento possível seria primeiro estimar por regressão linear ou logística um modelo para probabilidade uma observação pertencer à uma classe $f: \R^3 \to [0, \, 1]$ e depois alguma maneira de traduzir uma probabilidade presumida pelo modelo à uma classe $g: [0, \, 1] \to \{0, 1\}$. O modelo final, neste caso, seria a composição $g \circ f: \R^3 \to \{0, 1\} $.

O analista pode seguir por outro caminho e enumerar uma série de perguntas sobre cada observação antes de emitir um julgamento:
...
Cada pergunta aqui pode ser lida como uma função. 'O cliente tem menos de $k$ anos de idade?' é apenas $f(\text{idade}\, |\, k)$, tal que $f: \R_+ \to \{0, 1\}$. É de se esperar que o grupo de clientes entre 20 e 30 anos de idade cuja maior parte das compras históricas foram em eletrônicos e fazem compras acima da média tenha, entre seus membros, mais entusiastas de tecnologia do que usuários relutantes. Algumas perguntas são mais informativas que outras. Afinal, um usuário teve mais de 90\% das compras em eletrônicos é provavelmente mais relevante para a pergunta em questão do que saber se ele te mais ou menos do que 23 anos de idade. 
...
\begin{figure}
    \centering
    \includegraphics[scale = .65]{imagens/exemplo_arvore.png}
