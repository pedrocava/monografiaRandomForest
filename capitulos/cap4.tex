...
Afinal, cada árvore na floresta dá um voto contendo a sua classificação predita. A classe mais votada é dada como classe predita pela floresta. Podemos ler a fração dos votos que a classe mais votada levou como uma probabilidade predita e calcular os efeitos marginais da mesma maneira que faríamos em um caso de regressão. Essa mudança para que a saída do modelo seja em termos de probabilidade é trivial na maioria das implementações computacionais destes métodos e não leva a um aumento de complexidade da tarefa.
